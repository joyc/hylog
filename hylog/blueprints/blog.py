...
from hylog.forms import AdminCommentForm, CommentForm
from hylog.extensions import db
from hylog.utils import redirect_back
from hylog.emails import send_new_comment_email, send_new_reply_email
...
def show_category(category_id):
    ...
    per_page = current_app.config['HYLOG_POST_PER_PAGE']
...
def show_post(post_id):
    ...
    if current_user.is_authenticated:   # 如果当前用户已登录则使用管理员表单(扩展Flask-Login导入)
        form = AdminCommentForm()
        form.author.data = current_user.name
        form.email.data = current_app.config['HYLOG_EMAIL']
    ...
    if form.validate_on_submit():
        ...
        if replied_id:  # 如果URL中reply查询参数存在，说明是回复
        ...
        if current_user.is_authenticated:   # 根据登录状态显示不同的提示信息
            ...
            flash('评论已进入审核队列，审核通过后将显示在评论列表中。', 'info')
...
def change_theme(theme_name):
    if theme_name not in current_app.config['HYLOG_THEMES'].keys():
        abort(404)
